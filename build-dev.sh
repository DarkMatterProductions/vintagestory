...
#error_handling_execute "Installing pipenv" python3 -m pip install pipenv
error_handling_execute "Selecting Python ${LAVENDER}${PYTHON_VERSION}${NC}." pyenv local "${PYTHON_VERSION}"
error_handling_execute "Installing System Dependencies: ${LAVENDER}pipenv & requests${NC}." "python${PYTHON_SHORT_VERSION} -m pip install pipenv requests"
error_handling_execute "Installing Python Dependencies into Virtual Environment." "pipenv install --python ${PYTHON_SHORT_VERSION} -r ./vintage_rcon_client/requirements.txt"
error_handling_execute "Purging Local Git Tags." "git tag -d $(git tag -l)"
error_handling_execute "Pulling Git Tags." "git fetch origin --tags"
...
error_handling_execute "Generating Version with arguments: ${LAVENDER}${SEMVER_ARGS}${NC}" python ./semver.py "${SEMVER_ARGS}"
...
error_handling_execute "Building Docker image" sudo docker build --build-arg VS_VERSION_STATE="${VS_VERSION_STATE}" --build-arg VS_VERSION="${VS_VERSION}" --build-arg DOTNET_VERSION="${DOTNET_VERSION}" -t registry.dmpsys.in/vintagestory:"${VS_VERSION}-${DOCKER_VERSION_NEW}" .
error_handling_execute "Pushing (${LAVENDER}registry.dmpsys.in/vintagestory:${VS_VERSION}-${DOCKER_VERSION_NEW}${NC}) to Registry" sudo docker push registry.dmpsys.in/vintagestory:"${VS_VERSION}-${DOCKER_VERSION_NEW}"
step_header_string "Build Cleanup"
error_handling_execute "Pruning unused images" sudo docker image prune -f
error_handling_execute "Removing ${LAVENDER}build.env${NC} File" rm build.env
error_handling_execute "Removing ${LAVENDER}pipenv${NC} environment" pipenv --rm
step_footer_string
section_footer_string
